...
namespace Xinerji.Jwt
{
    ...
    public sealed class JwtEncoder : IJwtEncoder
    {
        ...
        public string Encode<T>(T payloadObject, string key)
        {
            DataContractJsonSerializer serializer = new DataContractJsonSerializer(payloadObject.GetType());
            MemoryStream ms = new MemoryStream();
            serializer.WriteObject(ms, payloadObject);
            string xpayload = Encoding.UTF8.GetString(ms.ToArray());
            ...
            var payloadBytes = GetBytes(xpayload);
            ...
            var signature = _algorithm.Sign(GetBytes(key), bytesToSign);
            segments.Add(_urlEncoder.Encode(signature));

            return String.Join(".", segments.ToArray());

           // return Encode(null, xpayload, key != null ? GetBytes(key) : null);
        }
