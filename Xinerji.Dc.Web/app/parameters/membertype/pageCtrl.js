...
mainapp.controller('sectionCtrl', ['$scope', 'utilities', '$http', '$templateCache', '$location',
    function ($scope, utilities, $http, $templateCache, $location) {
        ...
        $scope.Save = function () {
            ...
            var tempJsonRequest = {
                'MemberType': $scope.form
            };

            console.log(tempJsonRequest);

            $scope.warningMsg = "";
            if ($scope.form.Type.trim() == '') {
            ...
            if ($scope.warningMsg.trim() == '') {
                ...
                if ($scope.savingType == 1) {
                    $scope.url = jsonServiceURL + "/parameter/insertmembertype";
                }
                else {
                    $scope.url = jsonServiceURL + "/parameter/editmembertype";
                }
                $scope.method = "POST";
        ...
        $scope.Delete = function (form) {
            ...
            $scope.method = "POST";
            $scope.url = jsonServiceURL + "/parameter/deletemembertype";
            $http({
                method: $scope.method, url: $scope.url, data: tempJsonRequest
            }).
                then(function (response) {
        ...
        $scope.GetUserLevel = function (i) {

            if (i == 1) {
                return $scope.bundle.membertype.super;
            }
            else if (i == 2) {
                return $scope.bundle.membertype.admin;
            }
            else if (i == 3) {
                return $scope.bundle.membertype.lastuser;
            }
        }
}]);
